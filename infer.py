...
def infer(args:Config):
    ...
    pipe = StableDiffusionFaceAdapterPipeline.from_pretrained(
        ...
        local_files_only=args.use_cache,
    ...
    vae_ft_mse = AutoencoderKL.from_pretrained(
        "stabilityai/sd-vae-ft-mse", cache_dir=args.cache_dir if args.use_cache else None, torch_dtype=weight_dtype, local_files_only=args.use_cache
    ).to(device)
    pipe.vae = vae_ft_mse
    ...
    bfm_facemodel = bfm.BFM(
        focal=1015 * 256 / 224, image_size=256, bfm_model_path=os.path.join(args.checkpoint, "third_party/BFM_model_front.mat")
    ).to(device)
    ...
    net_image2token = Image2Token(
        visual_hidden_size=net_vision_encoder.vision_model.config.hidden_size, text_hidden_size=768, max_length=77, num_layers=3
    ).to(device)
    ...
    app = FaceAnalysis(
        name="antelopev2", root=os.path.join(args.checkpoint, "third_party"), providers=["CUDAExecutionProvider", "CPUExecutionProvider"]
    )
    ...
    for src_im_file in src_img_list:
        ...
        for drive_im_file in tqdm(natsorted(drive_img_list)):
            ...
            if os.path.isfile(accurate_mask_path):
                ...
                masks_tar = torch.argmax(
                    F.interpolate(seg_pred, [test_image_size, test_image_size], mode="bilinear"), dim=1, keepdim=True
                )  # torch.Size([1, 1, 512, 512]) 0-20
                face_masks_tar = (
                    torch.logical_or((masks_tar > 0) * (masks_tar < 7), (masks_tar > 9) * (masks_tar < 14)).float() - (masks_tar == 6).float()
                )
            ...
            # im_rgb_pil.save(os.path.join(save_concat_path, f"{drive_im_name}.jpg"), quality=100)
...
class FaceAptDataLoader(ExpDataLoader):
    def __init__(self):
        super().__init__("faceapt")
    
    def infer(self, source, target, output):
        config = Config()
    ...
    def run_video(self, source_img_path, target_video_path, out_video_path):
        ...
        base_dir = os.path.join(image_loader.base_dir, f"faceapt_{get_pid(source_img_path)}")
        source_image_dir = get_sub_dir(base_dir, "source")
        shutil.copy(source_img_path, source_image_dir)
        out_dir_path = get_sub_dir(base_dir, "output")
        self.infer(source_image_dir, image_loader.ori_imgs_dir, out_dir_path)
        return self.merge_video(get_sub_dir(out_dir_path, "swap"), out_video_path)
    

def main():
    loader = FaceAptDataLoader()
    loader.run_all()
    # for pair in loader.get_all_pairs():
    #     print(pair)
